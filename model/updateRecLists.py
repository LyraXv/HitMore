...
def updateGroundTruthFile(filepaths,approach,dataset):
    ...
    with open(gtNotFoundInfoPath, 'w', encoding='utf-8') as file:
        for row in FileNotFound:
            file.write(row + '\n')
...
if __name__ == '__main__':
    ...
    for index in range(3):
        ...
        for dataset,file in configx.filepath_dict.items():
            ...
            updateRecLists(file['bugFixedFileRanks'][index],dataset,configx.approach[index])
            print(f">>>>>{configx.approach[index]} RecLists of {dataset} is updated!<<<<<")
            # update gt
